...
def generate_pdf(bill_data):
    ...
    def bulletproof_split(text, fontName, fontSize, maxWidth):
        words = text.split(' ')
        lines = []
        current_line = ""
        for word in words:
            # If a single word is insanely long, split it char by char
            if stringWidth(word, fontName, fontSize) > maxWidth:
                if current_line:
                    lines.append(current_line)
                    current_line = ""
                temp_word = ""
                for char in word:
                    if stringWidth(temp_word + char, fontName, fontSize) <= maxWidth:
                        temp_word += char
                    else:
            ...
            if stringWidth(test_line, fontName, fontSize) <= maxWidth:
    ...
    raw_address = str(bill_data.get('address', '')).replace('\n', ' ').strip()
    
    # We add a 15 point safety buffer to guarantee it never overlaps Wtsp. No.
    max_addr_w_1 = (width - MARGIN - 190) - (MARGIN + 40) - 15
    max_addr_w_full = (width - MARGIN - 10) - (MARGIN + 40)
    
    addr_lines = []
    if raw_address:
        temp_lines = bulletproof_split(raw_address, "Helvetica", 11, max_addr_w_1)
        if temp_lines:
            addr_lines.append(temp_lines[0])
            if len(temp_lines) > 1:
                remainder = " ".join(temp_lines[1:]).strip()
                if remainder:
                    rest_lines = bulletproof_split(remainder, "Helvetica", 11, max_addr_w_full)
                    addr_lines.extend(rest_lines)
        ...
        if i < len(items):
            ...
            if desc:
                for line in desc.split('\n'):
                    desc_lines.extend(bulletproof_split(line, "Helvetica", 8, 220))
