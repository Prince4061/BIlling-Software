...
def get_analytics_data():
    ...
    # One row per bill for bill-level KPIs
    df_bills = df.drop_duplicates(subset=['Bill No'])
    
    # KPIs
    kpis = {
        "total_revenue": float(df_bills['Grand Total'].sum()),
        "total_received": float(df_bills['Paid Amount'].sum()),
        "total_due": float(df_bills['Due Amount'].sum()),
        "total_tax": float(df_bills['Tax'].sum()),
    ...
    df_bills_trend = df_bills.copy()
    try:
        df_bills_trend['ParsedDate'] = pd.to_datetime(df_bills_trend['Date'].astype(str).str.slice(0, 10), format='%d-%m-%Y', errors='coerce')
    except Exception:
        df_bills_trend['ParsedDate'] = pd.NaT
        
    df_bills_trend = df_bills_trend.dropna(subset=['ParsedDate'])
    ...
    for d in last_7_days:
        daily_rev = df_bills_trend[df_bills_trend['ParsedDate'].dt.date == d]['Grand Total'].sum()
        trend_revenue.append(float(daily_rev))
        
    # Top Items (By Qty)
    top_items_df = df.groupby('Item')['Qty'].sum().sort_values(ascending=False).head(5)
    
    # Company share
    company_df = df.groupby('Company')['Total Amount'].sum().sort_values(ascending=False)
    ...
    return {
        ...
        "top_items": {
            "labels": top_items_df.index.tolist(),
            "quantities": [float(x) for x in top_items_df.values.tolist()]
        },
        "companies": {
            "labels": company_df.index.tolist(),
            "revenue": [float(x) for x in company_df.values.tolist()]
        }
    }
